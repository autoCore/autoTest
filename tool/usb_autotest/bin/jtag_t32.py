...
def jtat_connect():
	...
	if (platform.system()=='Windows') or (platform.system()[0:6]=='CYGWIN') :
	  ...
	  if ctypes.sizeof(ctypes.c_voidp)==4:
		# Linux 32bit
		t32api = ctypes.CDLL("./tool/usb_autotest/libs/t32api.so")
		print ("Load 32bits so")
	  else:
		# Linux 64bit
		t32api = ctypes.CDLL("./tool/usb_autotest/libs/t32api64.so")

	t32api.T32_Config(b"NODE=",cfg.t32_IP)
	t32api.T32_Config(b"PORT=",cfg.t32_PORT)
	t32api.T32_Config(b"PACKLEN=",cfg.t32_PACKLEN)
