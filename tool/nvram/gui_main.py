...
class autoframe(wx.Frame):
    def __init__(self, leight, high):
        wx.Frame.__init__(self, None, -1, title="NVRAM TOOL",
                          size=(leight, high))
        self.file_name = None
        self.UI_init()
    ...
    def UI_init(self):
        ...
        fileMenu = wx.Menu()
        newItem = wx.MenuItem(fileMenu, id=wx.ID_NEW,
                              text='New', kind=wx.ITEM_NORMAL)
        fileMenu.Append(newItem)

        fileMenu.AppendSeparator()

        fileOpenItem = wx.MenuItem(
            fileMenu, id=wx.ID_OPEN, text='Open', kind=wx.ITEM_NORMAL)
        fileMenu.Append(fileOpenItem)

        quit = wx.MenuItem(fileMenu, id=wx.ID_EXIT,
                           text="Quit\tCtrl+Q", kind=wx.ITEM_NORMAL)
